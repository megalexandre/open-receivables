...
import 'package:organizagrana/shared/errors/api_error_code.dart'
    show ApiErrorCode, ErrorParams;

import 'package:http/http.dart' as http;

enum ApiFailureType { network, unauthorized, server, invalidResponse, validation }
...
class ApiValidationError {
  ...
  factory ApiValidationError.fromJson(Map<String, dynamic> json) =>
      ApiValidationError(
        errorCode: ApiErrorCode.fromCode(json['code'] as String? ?? ''),
...
class HttpApiClient {
  ...
  void _validateStatus(http.Response response) {
    ...
    if (response.statusCode == 422) {
      final decoded = jsonDecode(response.body);
      final raw = decoded is Map ? (decoded['errors'] as List?) ?? [] : [];
      throw ApiValidationException(
        raw.whereType<Map<String, dynamic>>().map(ApiValidationError.fromJson).toList(),
      );
    }

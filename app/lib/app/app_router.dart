...
import 'package:organizagrana/features/connections/presentation/pages/connections_page.dart';
import 'package:organizagrana/features/caixa/data/caixa_service.dart';
import 'package:organizagrana/features/caixa/presentation/pages/caixa_page.dart';
import 'package:organizagrana/features/generate_invoices/data/generate_invoices_service.dart';
import 'package:organizagrana/features/generate_invoices/presentation/pages/generate_invoices_page.dart';
import 'package:organizagrana/features/water_quality/data/water_quality_service.dart';
import 'package:organizagrana/features/water_quality/presentation/pages/water_quality_page.dart';
...
class AppRouter {
  AppRouter(this._session, {
    ...
    required this.generateInvoicesService,
  });
  ...
  static const String waterQualityPath = '/water-quality';
  static const String caixaPath = '/caixa';
  static const String generateInvoicesPath = '/generate-invoices';
    ...
    routes: [
      ...
      ShellRoute(
        ...
        routes: [
          ...
          GoRoute(
            path: waterQualityPath,
            pageBuilder: (context, state) => NoTransitionPage(
              child: WaterQualityPage(service: waterQualityService),
            ),
          ),
          GoRoute(
            path: caixaPath,
            pageBuilder: (context, state) => NoTransitionPage(
              child: CaixaPage(service: caixaService),
            ),
          ),
          GoRoute(
            path: generateInvoicesPath,
            pageBuilder: (context, state) => NoTransitionPage(
              child: GenerateInvoicesPage(
                service: generateInvoicesService,
                addressesService: addressesService,
              ),
            ),
          ),
...
class _AppShellState extends State<_AppShell> {
  ...
  int get _selectedIndex {
    for (var i = 0; i < _menuItems.length; i++) {
      ...
      if (item.isItem && widget.currentPath.startsWith('/${item.id}')) return i;
    }
    return 0;
  }

  void _onMenuSelect(int index) {
    if (index < _menuItems.length && _menuItems[index].isItem) {
